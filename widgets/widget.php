...
class SharelinkWidget extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        $get = mysql_query("select * from " . $wpdb->prefix . "sharelink order by created desc limit ".$options["widgetlimit"]);

        while ($result = mysql_fetch_array($get)) {

            
            $content .= "<li>";
            $content .= "<span class=\"sl-date\">" . date($options["widgetdate"], strtotime($result["created"])) . "</span> ";
